...
var PF_CTRL_ALL    = "All Frames";   // 체크 시 워크에어리어 전체 프레임 샘플링
var PF_MIN_SIZE    = 4;
...
function _pfEnsureCtrl(comp) {
    ...
    var cb = n.property("ADBE Effect Parade").addProperty("ADBE Checkbox Control");
    cb.name = PF_CTRL_ALL;
    cb.property(1).setValue(0);
    n.comment = "pad=0;all=0";
    return n;
}
function _pfCtrlAll(ctrl) {
    try { var v = ctrl.property("ADBE Effect Parade").property(PF_CTRL_ALL).property(1).value; return (v === true || v === 1); } catch (e) { return false; }
}
...
function _pfFitOne(target, warns) {
    var ctrl = _pfEnsureCtrl(target);
    var margin = _pfCtrlPad(ctrl);
    var mode = _pfCtrlAll(ctrl) ? "all" : "current";
    ...
    ctrl.comment = "pad=" + margin + ";all=" + (mode === "all" ? 1 : 0);
    return { pad: margin, mode: mode, name: target.name, from: from, to: [w, h], offset: [dx, dy], instances: snaps.length };
}
...
function fitPrecomp() {
    ...
    try {
        for (var t = 0; t < targets.length; t++) {
            var r = _pfFitOne(targets[t], warns);
    ...
    return ok({ fitted: results, warnings: warns, fromOutside: fromOutside });
}
...
function cropPollState() {
    try {
        ...
        var pad = _pfCtrlPad(ctrl), all = _pfCtrlAll(ctrl);
        var m = /pad=([-\d.]+)(?:;all=(\d))?/.exec(String(ctrl.comment || ""));
        var applied = m ? parseFloat(m[1]) : -1, appliedAll = m && m[2] ? (m[2] === "1") : false;
        var dirty = Math.abs(pad - applied) > 0.05 || all !== appliedAll;
        return ok({ active: true, comp: target.name, pad: pad, all: all, applied: applied, dirty: dirty });
    } catch (e2) {
...
function alignLayers(axis, mode, ref) {
    ...
    // 선택에 3D 레이어가 있을 때만 프로브를 3D 로 (Position 이 3차원이어야 toWorld/fromWorldVec 의 Z 가 보존됨).
    // 2D 만 선택했을 땐 2D 프로브 → 뷰어에 3D 축 표시가 스쳐 지나가지 않고 toComp 만 쓴다.
    var any3D = false;
    for (var s3 = 0; s3 < sel.length; s3++) { try { if (sel[s3].threeDLayer) any3D = true; } catch (e3) {} }
    var probe = comp.layers.addNull();
    probe.name = "__BANG_AL_PROBE__";
    if (any3D || ai === 2) probe.threeDLayer = true;
    var probeProp = probe.property("ADBE Transform Group").property("ADBE Position");
    try {
        // 1) 측정
        var items = [];
...
// ── Cloner Path: 대상 레이어에 BANG Cloner 를 넣고 Layout = Path, Path Layer = 셰이프 레이어 (패스 읽기는 플러그인이 AEGP 로 직접) ──
function _clShapePaths(layer) {   // 펜 패스·사각형·타원 유무 (경로 소스 판별용)
    var out = [];
    function walk(group) {
        for (var i = 1; i <= group.numProperties; i++) {
            var pr = group.property(i);
            if (pr.matchName === "ADBE Vector Group") walk(pr.property("ADBE Vectors Group"));
            else if (pr.matchName === "ADBE Vector Shape - Group" || pr.matchName === "ADBE Vector Shape - Rect" || pr.matchName === "ADBE Vector Shape - Ellipse") out.push(pr);
        }
    }
...
function clonerPathFromShape(targetIndex, shapeIndex) {
    ...
    if (_clShapePaths(shape).length === 0) return err("셰이프 레이어에 패스(펜 경로·사각형·타원)가 없습니다.");
    ...
    try { fx.property("Path Layer").setValue(shape.index); } catch (e2) { return err("Path Layer 를 설정하지 못했습니다: " + e2.toString()); }
    return ok({ paths: _clShapePaths(shape).length, target: target.name, shape: shape.name });
}
...
function applyNativeEffect(matchName) {
    ...
    if (matchName === "BANG Cloner" && layers.length === 2) {
        ...
        if (a instanceof ShapeLayer && !hasCloner(a) && (!(b instanceof ShapeLayer) || hasCloner(b))) { shapeL = a; targetL = b; }
            ...
            // 둘 다 셰이프면: 채우기(Fill)가 없는 쪽 → 경로 소스. 둘 다 채우기가 있으면 펜 패스가 있는 쪽 → 경로 소스
            function hasFill(L) { var found = false; (function walk(g) { for (var i = 1; i <= g.numProperties; i++) { var pr = g.property(i); if (pr.matchName === "ADBE Vector Group") walk(pr.property("ADBE Vectors Group")); else if (pr.matchName === "ADBE Vector Graphic - Fill") found = true; } })(L.property("ADBE Root Vectors Group")); return found; }
            function hasPen(L) { var ps = _clShapePaths(L); for (var i = 0; i < ps.length; i++) if (ps[i].matchName === "ADBE Vector Shape - Group") return true; return false; }
            var fa = hasFill(a), fb = hasFill(b);
            if (!fa && fb) { shapeL = a; targetL = b; }
            else if (!fb && fa) { shapeL = b; targetL = a; }
            else if (hasPen(a) && !hasPen(b)) { shapeL = a; targetL = b; }
            else if (hasPen(b) && !hasPen(a)) { shapeL = b; targetL = a; }
        }
        if (shapeL !== null) {
