...
- **Gradient (네이티브 이펙트 `BANG Gradient`)** — 정지점 최대 8개(색·위치·불투명도)짜리 그라데이션. 모양은 `Linear`·`Radial`·`Angular`·`Diamond`·`Reflected`·**`Contour`**(알파 경계까지의 거리 — 글자·도형 윤곽을 따라 흐릅니다), 보간은 `sRGB`·`Linear`·**`OKLab`**·`OKLCh`(색상환), `Repeat/Mirror`·`Cycles`·`Phase`·`Reverse`, 띄 없애는 `Dither`, 원본과의 블렌드·`Preserve Alpha`. 8/16/32bpc.
- **Stroke (네이티브 이펙트 `BANG Stroke`)** — 레이어 스타일 Stroke 의 대체. 알파 경계까지의 거리로 획을 그립니다. **획 하나 = 이펙트 하나**, 이펙트를 두 번 이상 적용하면 **앞 획 바깥에** 쌓여 동심 러리가 됩니다(`Position` Outside/Center/Inside · `Corner` **Round/Miter/Bevel** + `Miter Limit` · `Width` · `Offset` · `Softness` · `Opacity` · `Blend`), 색은 **단색 또는 그라데이션**(Across Stroke / Linear / Radial · Angle · Scale · **Opacity A/B** · Reverse), **Edge Noise**(Amount·Scale·Detail·Evolution·Seed)로 거친 가장자리, `Body`(Keep/Hide)·`Body Opacity`·`Order`. 레이어 경계 밖의 획도 잘리지 않습니다. 8/16/32bpc.
